...
1. Reads `claudeCodeFirstTokenDate` (legacy) or `firstStartTime` (current) from the Claude config backup files. This is the date you first used Claude Code. With multiple accounts, the earliest date across all accounts is used.
